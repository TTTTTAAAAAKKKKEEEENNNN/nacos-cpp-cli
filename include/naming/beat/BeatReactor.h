...
public:
	...
	: _namingProxy(namingProxy), _threadCount(threadCount), _beatInfoLock(), _stop(true), _clientBeatInterval(5 * 1000)
	{
	...
	: _namingProxy(namingProxy), _threadCount(UtilAndComs::DEFAULT_CLIENT_BEAT_THREAD_COUNT), _beatInfoLock(), _stop(true), _clientBeatInterval(5 * 1000)
	{
		_threadPool = new ThreadPool("HeartbeatDaemonPool", _threadCount);
		_beatMaster = new Thread("BeatMaster", beatMaster, this);
	};
